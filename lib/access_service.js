...
models.forEach(function(model) {
	// one model: .topic()
	var camelName = _.camelCase(model);
	Service.prototype[camelName] = function(params) {
		return this.one(model, params);
	};
	// list model: .topics()
	Service.prototype[camelName + 's'] = function(params) {
		return this.list(model, params);
	};
	// count model: .countTopics()
	Service.prototype['count' + camelName + 's'] = function(params) {
